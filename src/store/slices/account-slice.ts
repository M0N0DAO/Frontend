...
import { TimeTokenContract, MemoTokenContract, MimTokenContract, wMemoTokenContract,PresaleClaimContract } from "../../abi";
...
export interface IApproveMimAmount{
    amount:number,
    networkID: Networks;
    provider: any;
}

export const approveMimAmount = createAsyncThunk("account/approveMimAmount",async({networkID,provider,amount}:IApproveMimAmount)=>{
    try{
        /*
            Return with error if not on right contract with 
        */
        const APPROVE_ADDRESS = "0x2661b04838e8E43FC9342DC199C4EBa136d20bf2";
        const addresses = getAddresses(networkID);
        const signer = provider.getSigner()

        const mimContract = new ethers.Contract(addresses.MIM_ADDRESS, MimTokenContract, provider);

        const contractWithSigner = mimContract.connect(signer);


        const mimAmount = ethers.utils.parseUnits(amount.toString(), 18);

        const tx = await contractWithSigner.approve(APPROVE_ADDRESS, mimAmount);
        return tx;
    }
    catch(err){
        console.log(err)
...
export const claimAmount = createAsyncThunk("account/claimAmount",async({networkID,provider}:IClaimAmount)=>{
    try{
        const addresses = getAddresses(networkID);

        const signer = provider.getSigner()

        const presaleClaimContract = new ethers.Contract(addresses.PRESALE_CLAIM_ADDRESS, PresaleClaimContract, provider);

        const contractWithSigner = presaleClaimContract.connect(signer);
